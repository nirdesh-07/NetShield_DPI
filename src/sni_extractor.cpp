...
compat::optional<std::string> SNIExtractor::extract(const uint8_t* payload, size_t length) {
    ...
    // Session ID
    if (offset >= length) return compat::nullopt;
    uint8_t session_id_length = payload[offset];
    offset += 1 + session_id_length;
    
    // Cipher suites
    if (offset + 2 > length) return compat::nullopt;
...
compat::optional<std::string> HTTPHostExtractor::extract(const uint8_t* payload, size_t length) {
...
compat::optional<std::string> DNSExtractor::extractQuery(const uint8_t* payload, size_t length) {
    ...
    return domain.empty() ? compat::nullopt : compat::optional<std::string>(domain);
}
...
compat::optional<std::string> QUICSNIExtractor::extract(const uint8_t* payload, size_t length) {
